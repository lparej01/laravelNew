...
<body>
  ...
  <div class="container-fluid py-4">   
    <table>
      <tr>
          ...
          <h3>Reporte de existencia por costo categorías  </h3>
          </td>
      </tr>
    </table>  
  </div>
  <div class="container-fluid py-4">
    <header>
      <h2>Existencia de productos según su costos categorías</h2>
  </header>
      ...
      <tbody>
          @foreach ($existencia as $item)
              <tr><td>{{$item->catId}}</td>
                  ...
                  <td style="width: 60px">{{$item->periodo}}</td>                            
                  <td style="width: 80px">{{$item->invFinal}}</td>
                  <td style="width: 60px" >{{$item->costoUnitario}}</td>
              </tr>
          @endforeach                           
      </tbody>
